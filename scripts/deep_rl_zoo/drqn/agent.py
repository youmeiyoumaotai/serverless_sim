...
class Drqn(types_lib.Agent):
    ...
    def _choose_action(self, timestep: types_lib.TimeStep, epsilon: float) -> types_lib.Action:
        ...
        s_t = torch.from_numpy(timestep.observation[None, None, ...]).to(device=self._device, dtype=torch.float32)
        hidden_s = tuple(s.to(device=self._device) for s in self._lstm_state)
        network_output = self._online_network(s_t, hidden_s)
    ...
    def _calc_loss(self, transitions: replay_lib.Transition) -> torch.Tensor:
        ...
        a_tm1 = torch.from_numpy(transitions.a_tm1).to(device=self._device, dtype=torch.int64)  # [B, T]
        r_t = torch.from_numpy(transitions.r_t).to(device=self._device, dtype=torch.float32)  # [B, T]
        s_t = torch.from_numpy(transitions.s_t).to(device=self._device, dtype=torch.float32)  # [B, T, state_shape]
        done = torch.from_numpy(transitions.done).to(device=self._device, dtype=torch.bool)  # [B, T]

        # Rank and dtype checks, note we have a new unroll time dimension, states may be images, which is rank 5.
        base.assert_rank_and_dtype(s_tm1, (3, 5), torch.float32)
        ...
        hidden_s = tuple(s.to(device=self._device) for s in hidden_s)
        target_hidden_s = tuple([hx.clone().to(device=self._device) for hx in hidden_s])
        ...
        B, T = s_tm1.shape[:2]
        q_tm1 = q_tm1.view(B * T, -1)
        a_tm1 = a_tm1.view(B * T)
        r_t = r_t.view(B * T)
        discount_t = discount_t.view(B * T)
        target_q_t = target_q_t.view(B * T, -1)
        q_t_selector = q_t_selector.view(B * T, -1)

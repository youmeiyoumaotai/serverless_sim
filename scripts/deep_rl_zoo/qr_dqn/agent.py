...
class QRDqn(types_lib.Agent):
    ...
    def __init__(
        ...
        base.assert_rank_and_dtype(quantiles, 1, torch.float32)
        ...
        self._num_taus = quantiles.size(0)
        self._quantiles = quantiles.to(device=self._device)
    ...
    def _calc_loss(self, transitions: replay_lib.Transition) -> Tuple[torch.Tensor, np.ndarray]:
        ...
        priorities = loss.detach().cpu().numpy()  # [batch_size]

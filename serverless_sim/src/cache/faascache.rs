...
pub fn register_fn_profile(fnid: FnId, init_cost: f32, size: f32) {
    let mut registry = profile_registry().lock().unwrap();
    registry.insert(
        fnid,
        FnProfile {
            init_cost: init_cost.max(1.0),
            size: size.max(1.0),
        },
    );
}
...
impl FaasCache {
    ...
    fn compute_priority(&self, fnid: FnId, freq: u64) -> f32 {
        ...
        self.clock + (freq as f32) * profile.init_cost / profile.size
    }

    fn touch_entry(&mut self, fnid: FnId) {
        let freq = {
            let freq = self.frequencies.entry(fnid).and_modify(|v| *v += 1).or_insert(1);
            *freq
        };
...
impl InstanceCachePolicy<FnId> for FaasCache {
...
unsafe impl Send for FaasCache {}
